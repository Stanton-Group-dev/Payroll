...
| Workyard single-set language (since per-worker is impossible, 0.12): keep **bilingual** (live now, per 0.10) or switch to **Spanish-only** (cleanest for the crew; our app already renders English for billing via `activityOf`)? | The one Workyard set must pick a lane. | Depends on whether anyone outside the Spanish-speaking field crew reads the Workyard tap list. |
