...
| 0.17 | **Office/overhead work is identified by the COST CODE (what we actually mean by on-site office work), NOT by geofence/location.** The office geofence is not a reliable signal: it sits **next to a hardware store** (the geofence covers the store), the **construction team is sometimes physically in the office** to plan/review, and **all construction + maintenance staff must clock in at the office** while inside that geofence. So "inside the office geofence" ≠ "office overhead work." Keep identifying office-overhead by cost code / project (current `SPREAD_OVERHEAD_PROJECT_NAMES`), broadened enough to stay valuable; do **not** add a hard geofence→overhead rule. | A location rule would misclassify field crew (who clock in at the office and shop at the adjacent hardware store) as office overhead and smear their cost across the portfolio. | Conversation 2026-06-20 | **active** |
| 0.18 | **Among non-engineering work, the unallocated-hours SMS is the top priority** — daily auto-send + reworded self-service copy ("you have X unallocated hours, fix them in Workyard"). The engine already exists (§0.5, §4); it needs the cron + the new copy. | Field-pay disputes are the biggest people-cost; early self-service notice is the fix. | Conversation 2026-06-20 (Alex) | **active** (priority) |
| 0.19 | **The Tenant-Document-Coordination ROI report is OUT of payroll scope** — it's a cross-module ops/leasing analytics idea (which tenants/documents are chronic at chasing paperwork), not payroll; needs leasing-module data. Drop/park it from this project. Separately, the **invoice label** for OFFICE-coded time was **renamed 2026-06-20 to "Office & Administrative Support"** (was the misleading "Tenant Coordination"/"Office") — see §5. | User flagged it as out of place; the PRD itself puts tenant-level attribution out of payroll scope. | Conversation 2026-06-20 (Alex); `TENANT_DOCUMENT_COORDINATION_PRD.md:38,42` | **active** (report dropped; label renamed) |
...
| **Pre-fund is a REFERENCE figure only** = Σ gross + Σ(gross×0.08 tax) + Σ(gross×0.03 WC); shown before ADP export. In practice the **full amount is transferred** (gross + the 8% tax + 3% WC estimates), so the prefund formula's omissions (e.g. management fee not included) are **cosmetic, not cash-impacting** — drops that fix to low priority. **Caveat:** because the transfer is built on system gross, any gross understatement (e.g. OT funded at straight time) shorts the transfer by that amount **plus its 8%+3%**. (Wk 2026-06-07: OT premium $447.64 → ~$496.86 transfer short incl. tax/WC.) | `PAYROLL_PRD.md:193-197`; Conversation 2026-06-20 | active |
...
| **What the management fee IS:** Stanton's fee for running payroll + property management for those properties — i.e., **billed revenue**, not an internal estimate. This is why the fee *total* must be the correct property-by-property figure: it's what each property is actually charged. | Conversation 2026-06-20 (Alex) | active |
...
| On invoices, office-coded labor (`OFFICE`/`Oficina`) shows as **"Office & Administrative Support"** — general back-office work **plus** on-site admin done at properties, billed as overhead support. **Renamed 2026-06-20** from the misleading "Tenant Coordination"; note the code had actually been emitting plain "Office" (the PRD's "Tenant Coordination" never shipped). | `cost-code-breakdown.ts:31` (`activityOf`); Conversation 2026-06-20 | active |
...
| **Week locking is an operator-controlled SWITCH** (manual lock/unlock toggle), not only an automatic by-product of the approval chain — the operator locks a finished week and can unlock it to make changes. Reframes the open approval-locking work toward a simple switch rather than complex sequential-stage enforcement. | Conversation 2026-06-20 (Alex) | active |
...
| **On-site office/property classification refinement** — strengthen how on-site office work vs. property work is distinguished (beyond the cost-code rule in §0.17). **Becomes a near-term blocker once a few more on-site property-management staff are hired**; the cost-code approach suffices until then. | Conversation 2026-06-20 | future (gated on headcount) |
| **Split the `OFFICE` cost code into distinct codes** for: (a) **working in the office** (back-office overhead), (b) **clocking in at the office** geofence (a punch location only — not billable office work), and (c) **office/admin work done at a tenant's property**. Today all three collapse into one `OFFICE` code, so the invoice label and the overhead-vs-property split are approximate. Pairs with the on-site classification refinement above and §0.17. (Workyard cost codes are created manually — API can't.) | Conversation 2026-06-20 (Alex) | future |
| **Soft geofence clock-in SMS** — when a worker clocks in outside the correct geofence, send an SMS nudge. **Soft rule, not a hard block.** Sibling of the unallocated-hours SMS (§0.5/§4). | Conversation 2026-06-20 | future |
| **CEO/COO business-logic review doc** — generate a plain-language markdown listing **every business-logic choice made** (sourced from this log), formatted to walk through with the CEO + COO for discussion and sign-off. | Conversation 2026-06-20 (Alex) | future |
...
- **2026-06-20** — Added §0.17 (office identified by cost code, not geofence — geofence polluted by adjacent hardware store + mandatory office clock-ins) and three §8 future items (on-site office/property classification refinement, gated on hiring more on-site property-management staff; soft geofence clock-in SMS; CEO/COO business-logic review doc).
- **2026-06-20** — Added §0.18 (unallocated-hours SMS = top non-engineering priority), §0.19 (tenant-document-coordination ROI report dropped from payroll scope; invoice "Tenant Coordination" label under review), §1 prefund-is-reference-only clarification, §5 management-fee-is-billed-revenue clarification, and §6 week-locking-is-a-manual-switch.
- **2026-06-20** — Renamed the invoice label for office-coded labor from "Tenant Coordination" (never shipped; code emitted plain "Office") to **"Office & Administrative Support"** (`cost-code-breakdown.ts:31`). Added §8 future item to split `OFFICE` into three codes (work-in-office / clock-in-at-office / office-admin-at-property).
